...
class ApprovalRequestControllerIndexTest < ActionDispatch::IntegrationTest
  def setup
  ...
  test "should add requests and link to approver" do
    sign_in!
    assert_equal 0, ApprovalRequest.count
    post approval_requests_url, params: {ht_users: [@user1.email, @user2.email], submit_requests: true}
    assert_response :redirect
    follow_redirect!
    assert_equal 2, ApprovalRequest.count
    assert_match "Added", flash[:notice]
    assert_not_nil assigns(:incomplete_reqs)
    assert_not_nil assigns(:complete_reqs)
    assert_equal "index", @controller.action_name
    assert_select "a[href='#{edit_approval_request_path(@user1.approver)}']"
  end

  test "should get index page and fail to submit zero-length request list" do
    sign_in!
    post approval_requests_url, params: {submit_requests: true}
    assert_response :redirect
    follow_redirect!
    assert_not_nil assigns(:incomplete_reqs)
  ...
  test "should get index page and fail to submit approval request for unknown user" do
    sign_in!
    post approval_requests_url, params: {ht_users: ["nobody@nowhere.org"], submit_requests: true}
    assert_response :redirect
    follow_redirect!
    assert_not_nil assigns(:incomplete_reqs)
    assert_empty assigns(:incomplete_reqs)
...
class ApprovalRequestControllerEditTest < ActionDispatch::IntegrationTest
  def setup
    @user = create(:ht_user, approver: "approver@example.com")
    @req = create(:approval_request, userid: @user.email, approver: @user.approver)
  end

  test "should get edit page" do
    sign_in!
    get edit_approval_request_url @user.approver
    assert_response :success
...
class ApprovalRequestControllerUpdateTest < ActionDispatch::IntegrationTest
  def setup
    @user = create(:ht_user, approver: "approver@example.com")
    @req = create(:approval_request, userid: @user.email, approver: @user.approver)
  end
  ...
  def patch_approval_request(approver = @user.approver, params: {})
    sign_in!
    patch approval_request_url approver, params: params
    assert_response :redirect
...
class ApprovalRequestControllerShowTest < ActionDispatch::IntegrationTest
  def setup
    @user = create(:ht_user, approver: "approver@example.com")
  end
  ...
  test "should get show page" do
    sign_in!
    get approval_request_url @user.approver
...
class ApprovalRequestControllerResendTest < ActionDispatch::IntegrationTest
  def setup
    @user = create(:ht_user, approver: "approver@example.com")
    @req = create(:approval_request, userid: @user.email, approver: @user.approver, sent: Time.now - 30.days)
  end

  test "resending e-mail resets sent timestamp" do
    sign_in!
    patch approval_request_url @user.approver
    follow_redirect!
    assert_equal Date.parse(@req.reload.sent).to_s, Date.parse(Time.zone.now.to_s).to_s
  end
end

class ApprovalRequestControllerBatchRenewalTest < ActionDispatch::IntegrationTest
  def setup
    @user1 = create(:ht_user, approver: "approver@example.com")
    @user2 = create(:ht_user, approver: "approver@example.com")
    @req1 = create(:approval_request, userid: @user1.email, approver: @user1.approver, sent: Time.now - 30.days, received: Time.now - 1.day, renewed: nil)
    @req2 = create(:approval_request, userid: @user2.email, approver: @user2.approver, sent: Time.now - 30.days, received: Time.now - 1.day, renewed: nil)
  end

  test "should get index after renewing 2 users" do
    sign_in!
    assert_equal 2, ApprovalRequest.where(renewed: nil).count
    post approval_requests_url, params: {ht_users: [@user1.email, @user2.email], submit_renewals: true}
    assert_response :redirect
    follow_redirect!
    ...
    assert_equal 0, ApprovalRequest.where(renewed: nil).count
  ...
  test "should get index page and fail to submit zero-length renewal list" do
    sign_in!
    assert_equal 2, ApprovalRequest.where(renewed: nil).count
    post approval_requests_url, params: {submit_renewals: true}
    assert_response :redirect
    follow_redirect!
    assert_match "No users selected", flash[:alert]
    assert_equal "index", @controller.action_name
  ...
  test "should not renew if no existing request" do
    ...
    post approval_requests_url, params: {ht_users: [@user3.email], submit_renewals: true}
    assert_response :redirect
    follow_redirect!
...
class ApprovalRequestsControllerRolesTest < ActionDispatch::IntegrationTest
  def setup
    @req = create(:approval_request)
  end

  test "Admin user can see approval_requests index and edit requests" do
    sign_in! username: "admin@default.invalid"
    get approval_requests_url
    assert_response 200
    get edit_approval_request_url @req.approver
    assert_response 200
  end

  test "Staff user can see approval_requests index but not edit" do
  ...
  test "Institutions-only user cannot see approval_requests index or edit" do
...
class ApprovalRequestsControllerDeleteExpiredTest < ActionDispatch::IntegrationTest
  def setup
    @user1 = create(:ht_user, approver: "approver@example.com")
    @user2 = create(:ht_user, approver: "approver@example.com")
    @user3 = create(:ht_user, approver: "approver@example.com")
    @req1 = create(:approval_request, userid: @user1.email, approver: @user1.approver, sent: Time.now, received: nil, renewed: nil)
    @req2 = create(:approval_request, userid: @user2.email, approver: @user2.approver, sent: Time.now - 30.days, received: nil, renewed: nil)
    @req3 = create(:approval_request, userid: @user3.email, approver: @user2.approver, sent: Time.now - 30.days, received: Time.now - 2.days, renewed: nil)
  end

  test "should delete only one expired request" do
    sign_in!
    assert_equal 3, ApprovalRequest.count
    assert_equal 1, ApprovalRequest.expired.count
    post approval_requests_url, params: {delete_expired: true}
    assert_response :redirect
    follow_redirect!
    ...
    assert_raises ActiveRecord::RecordNotFound do
      ApprovalRequest.find @user2.email
    end
    assert_equal 2, ApprovalRequest.count
    assert_equal 0, ApprovalRequest.expired.count
  end
end
