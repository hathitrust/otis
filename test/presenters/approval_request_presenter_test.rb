...
class ApprovalRequestTest < ActiveSupport::TestCase
  ...
  test "approver link" do
    req = build(:approval_request)
    assert_match("a href=\"/approval_requests/#{req.approver}/edit\"", presenter(req).approver_link)
  end
end

class ApprovalRequestPresenterBadgeTest < ActiveSupport::TestCase
  def badge(req)
    ApprovalRequestPresenter.new(req).badge
  end
  ...
  test "sent badge" do
    req = build(:approval_request, :sent)
  ...
  test "unsent badge" do
    req = build(:approval_request, :unsent)
