...
class ApprovalRequestsController < ApplicationController
  ...
  def index
    requests = ApprovalRequest.order("approver")
    @incomplete_reqs = requests.not_renewed.map { |r| ApprovalRequestPresenter.new(r) }
    @complete_reqs = requests.renewed.map { |r| ApprovalRequestPresenter.new(r) }
  ...
  def fetch_requests
    @reqs = ApprovalRequest.for_approver(params[:id]).not_renewed.map do |r|
      ApprovalRequestPresenter.new(r)
    end
  end
  ...
  def add_requests(emails)
    ...
    adds = []
    emails.each do |e|
      next if ApprovalRequest.where(userid: e, renewed: nil).count.positive?
  ...
  def add_request(email)
    u = HTUser.find(email)
    ApprovalRequest.new(ht_user: u, approver: u.approver).save!
  end
  ...
  def delete_expired
    ApprovalRequest.expired.destroy_all.count
  end
end
