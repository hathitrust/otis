...
class ContactTypesController < ApplicationController
  ...
  def new
    @contact_type = ContactTypePresenter.new(ContactType.new)
  end

  def index
    contact_types = ContactType.order(:name)
    @contact_types = contact_types.map { |c| ContactTypePresenter.new(c) }
  end
  ...
  def create
    @contact_type = ContactType.new(contact_type_params(PERMITTED_CREATE_FIELDS))
    if @contact_type.save
      log
      redirect_to @contact_type, note: "Contact type successfully created"
    else
      flash.now[:alert] = @contact_type.errors.full_messages.to_sentence
      @contact_type = ContactTypePresenter.new(@contact_type)
      render :new
    end
  end
  ...
  def contact_type_params(permitted_fields)
    @contact_type_params ||= params.require(:contact_type)
      .permit(*permitted_fields)
      .transform_values! { |v| v.present? ? v : nil }
  end

  def fetch_contact_type
    @contact_type = ContactTypePresenter.new(ContactType.find(params[:id]))
  end
end
