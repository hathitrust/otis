...
class ApprovalRequest < ApplicationRecord
  ...
  scope :expired, -> { where(renewed: nil, received: nil).where.not(sent: nil).where("sent < ?", ApprovalRequest.expiration_date) }
  ...
  def expired?
    self[:sent].present? && self[:sent] < ApprovalRequest.expiration_date
  end
