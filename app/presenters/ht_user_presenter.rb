...
class HTUserPresenter < ApplicationPresenter
  ...
  ALL_FIELDS = %i[
    email userid displayname approver approver_name authorizer activitycontact
  ...
  # Show an abbreviated version in the index table so we don't take up so much real estate.
  def show_role
    if action == :index
      service_role.present? ? service_role.name : "<code>#{role}</code>".html_safe
    else
